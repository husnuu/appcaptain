  ...
  async function load() {
    ...
    try {
      ...
      if (err instanceof ApiError && err.status === 403) {
        setError("Bu sayfayı görüntülemek için yönetici (admin) yetkisi gerekiyor.");
      } else {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 border-b border-gray-200 pb-6 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-[28px] font-bold tracking-[-0.02em] text-[#111827]">İndirim Yönetimi</h1>
        ...
        <button
          onClick={openCreate}
          className="flex items-center gap-2 self-start rounded-xl bg-[#0097A7] px-5 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-[#00838F]"
        >
      ...
      {loading ? (
        ...
        <EmptyState onCreate={openCreate} hasAny={discounts.length > 0} disabled={!!error} />
      ) : (
        <DiscountTable
          discounts={filtered}
          busyId={busyId}
...
function DiscountTable({
  ...
  const th = "text-left text-xs font-semibold uppercase tracking-wider text-gray-500 px-4 py-4";
  return (
...
function DiscountModal({
  ...
  const targetOptions = isAdmin
    ...
    : TARGET_OPTIONS.filter(
        (o) => o.value === DiscountTarget.BOAT || o.value === DiscountTarget.EXPERIENCE
      );
        ...
        <div className="space-y-6 p-6">
          ...
          {form.target === DiscountTarget.BOAT && (
            <div>
              ...
              <OptionSelector kind="boat" value={form.boatId} onChange={(id) => set("boatId", id)} />
            </div>
          )}
...
function OptionSelector({
  ...
  useEffect(() => {
    ...
    fetcher
      ...
      .catch((err) => {
        if (!cancelled) setError(err instanceof ApiError ? err.message : "Liste yüklenemedi");
      })
      .finally(() => {
