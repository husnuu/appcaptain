...
export function toBlockResponseDTO(row: BookingCalendarBlockRow): BlockResponseDTO {
  return {
    id: row.id,
    boatId: row.boatId,
    model: (row.model as BookingModel) ?? null,
...
export type BookingCalendarBlockRow = {
  id: string;
  boatId: string;
  model: string | null;
  reason: string;
